...
export async function generateMetadata({
  ...
  if (!post) {
    return {
      title: 'Episode Not Found | RummerLab',
    };
  }

  const title = `${post.title} | Athletes of the Reef`;
  const description = post.excerpt || post.content.substring(0, 160);
  const url = `https://rummerlab.com/podcast/${sanitizeSlugForUrl(slug)}`;
      ...
      <header className="mb-8">
        ...
        {(post.journal || post.doi || post.podcast) && (
          <div className="mb-6 text-sm text-gray-600 dark:text-gray-400">
            ...
            {post.podcast && (
              <span className="font-medium">Featured on: {post.podcast}</span>
            )}
          </div>
        )}
